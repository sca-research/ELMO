...
//fprintf(stderr,"0x%08X = 0x%08X + 0x%08X\n",rc,ra,rb);
    ...
    if((inst&0xFE00)==0x1800)
    {
       ...
       // if(registerdataflow) print32(ra_keyflow);
        rb=read_register(rm);
        rb_keyflow = read_register_keyflow(rm);
       // if(registerdataflow) print32(rb_keyflow);
        rc=ra+rb;
        rc_keyflow = compute_keyflow(ra_keyflow, rb_keyflow, 0, 0);
        op1=ra; op2=rb;
        op1_keyflow = ra_keyflow; op2_keyflow = rb_keyflow;
        write_register(rd,rc);
        write_register_keyflow(rd,rc_keyflow);
       // if(registerdataflow) print32(rc_keyflow);
        do_nflag(rc);
        do_zflag(rc);
        do_cflag(ra,rb,0);
        do_vflag(ra,rb,0);
...
//fprintf(stderr,"0x%08X = 0x%08X + 0x%08X\n",rc,ra,rb);
    ...
    if((inst&0xFFC0)==0x4100)
    {
        ...
        if(registerdataflow) dataptr = update_dataflow(dataptr,0,0,5,op1_keyflow, op2_keyflow);
        return(0);
    }
    ...
    if((inst&0xF000)==0xD000)
    {
        ...
        op1 = 0; op2 = 0;if(registerdataflow) dataptr = update_dataflow(dataptr,op1,op2, 5, op1_keyflow, op2_keyflow);	
        return(0);
...
//fprintf(stderr,"sp 0x%08X\n",sp);
        ...
        for(ra=0,rb=0x01;rb;rb=(rb<<1)&0xFF,ra++)
        {
            if(inst&rb)
            {
                ...
                op1 = 0; op2 = 0;
                if(registerdataflow) dataptr = update_dataflow(dataptr,op1,op2, 5, op1_keyflow, op2_keyflow);            rd+=4;
            }
        }
        if(inst&0x100)
        {
    ...
    if((inst&0xF800)==0xC000)
    {
        ...
        for(ra=0,rb=0x01;rb;rb=(rb<<1)&0xFF,ra++)
        {
            if(inst&rb)
            {
                ...
                op1 = 0; op2 = 0;
        if(registerdataflow) dataptr = update_dataflow(dataptr,op1,op2, 5, op1_keyflow, op2_keyflow);                sp+=4;
            }
        }
